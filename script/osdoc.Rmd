...
attach(snc)  
snc=snc[which(topo!="Calota craniana"&histo!="Histiocitose"&histo!="Hemangiopericitoma"&histo!="Neuroblastoma"&histo!="Outros"),]
snc=droplevels(snc)
detach(snc)
```
...
levels(snc$histo.group)=c("Outros tumores benignos","Glioma de baixo grau","Glioma de alto
grau","Outros tumores malignos","Tumores de células germinativas","Ependimoma","Ependimoma
anaplásico","Outros gliomas","Meduloblastoma","Outros tumores embrionários","Tumor
teratóide-rabdóide","Sem histologia")
par(las=2)
par(mar=c(5,11,4,2))
...
```{r Figura5, fig.path='../fig/', fig.height=7, fig.width=7}
barplot(summary(as.factor(who.g))*100/sum(summary(as.factor(who.g))),col="white",
names.arg=c("I","II","III","IV","Sem histologia"),cex.names=0.8)
```
...
pander(Freq(topo,ord='desc'), style = 'rmarkdown')
```
